...
//
// Guarded by a module-level flag (not component state) so it fires exactly once per
// page session — deliberately NOT cancelled on effect cleanup, since React 18 StrictMode's
// dev-only mount→unmount→remount cycle would otherwise cancel these on the first
// (simulated) unmount and then skip rescheduling on remount, so the events would never fire.
let started = false
